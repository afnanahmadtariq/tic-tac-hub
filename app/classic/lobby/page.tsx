...
export default function ClassicLobby() {
  ...
  const startLocalGame = () => {
    ...
    router.push(
      `/classic?mode=local&playerX=${encodeURIComponent(playerXName)}&playerO=${encodeURIComponent(playerOName)}&start=${startingPlayer}`,
    )
  }

  const startCPUGame = () => {
    router.push(
      `/classic?mode=cpu&player=${encodeURIComponent(playerName)}&difficulty=${difficulty}&playerStarts=${playerStarts}`,
    )
  }
  ...
  const joinRoom = () => {
    if (roomCode.trim()) {
      router.push(`/classic?mode=online&room=${roomCode.trim()}`)
    }
  }
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div
                className={`w-10 h-10 bg-blue-500/10 rounded-lg flex items-center justify-center ${theme === "arcade" ? "glow" : ""}`}
              >
                <Grid3X3 className="w-6 h-6 text-blue-500" />
              </div>
              <div>
                <h1 className="text-2xl font-bold text-balance">Classic Tic Tac Toe – Pre-Game Lobby</h1>
                <p className="text-sm text-muted-foreground">Set up your game and get ready to play</p>
              </div>
            </div>
          ...
          <Tabs defaultValue="local" className="w-full">
            ...
            <TabsContent value="local" className="space-y-6">
              <Card className={`${theme === "arcade" ? "glow-card" : ""}`}>
                <CardHeader>
                  <CardTitle className="text-blue-500">Local Multiplayer</CardTitle>
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button onClick={startLocalGame} className="w-full bg-blue-500 hover:bg-blue-600" size="lg">
                    Start Local Game
                  </Button>
                </CardContent>
              </Card>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={difficulty} onValueChange={setDifficulty}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="expert">Expert - Unbeatable AI</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
